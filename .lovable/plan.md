# Retirer les formes que j'ai fabriquées, corriger les sens attestés, et poser une règle qui m'en empêche

Aucun nouveau document (pas d'ODT/PDF). Corrections des données en ligne uniquement.

## Ce qui s'est passé

Dans la leçon sur « Kue », une seule forme était attestée : **Kue kotela (ka)?**. À partir d'elle j'ai décliné une série par analogie — Kue ka?, Kue ba?, Kue lu? — avec des sens scolaires et des versions portugaises inventées, propagés ensuite dans le dictionnaire en ligne et le traducteur.

## 1. Suppression des formes fabriquées (non attestées)

Retirées de `data/dictionary-entries.json`, `src/data/lessons.ts` (lignes 12861, 12863 et 12992, 12994, plus le distracteur d'exercice ligne 18072) et `supabase/functions/translate-lari/index.ts` (lignes 3700, 3702) :

- **Kue ka?** (« Où va-t-elle à l'école ? »)
- **Kue lu?** (« Où allez-vous à l'école ? »)

## 2. Formes attestées — conservées, avec vos sens exacts
...
- **Kue ba ka?** = Où habites-tu ? / Where do you live? (les deux formes sont bonnes — conservées toutes les deux)
- **Kue ba kotela ka ?** = mot à mot « Où entrent-elles ? » ; sous-entendu, selon le contexte, « Où vont-elles à l'école ? »
- **Nzo mikanda aku kue ye(na)?** = Où est ton école ? / Where is your school?
- **Nzo mikanda andi kue ye(na)?** = Où est son école ? = Où va-t-elle à l'école ? / Where is her/his school?
- **Nzo mikanda eno kue ye(na)?** = Où est votre école ? / Where is your (pl.) school?
- **Nzo mikanda awu kue ye(na)?** = Où vont-elles à l'école ? ; mot à mot « Leur école, où est-elle ? »

Ces sens sont saisis tels quels, y compris les notes mot à mot, dans le dictionnaire, la leçon et le traducteur.
