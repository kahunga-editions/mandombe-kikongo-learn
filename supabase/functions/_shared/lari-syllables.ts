// Kikongo Lari syllable segmentation + alignment + coaching rules.
// Pure, dependency-free — usable in Deno edge functions and as Node/Vite import.

// ---------- Phonotactics --------------------------------------------------

/** Onset clusters legal in Lari. Order matters (longest first). */
...
/**
 * Naive Lari syllabifier (CV / CCV / CVN). Greedy left-to-right.
 *   bujitu  -> bu-ji-tu
 *   nzila   -> nzi-la
 *   nsoneka -> nso-ne-ka
 *   mbote   -> mbo-te
...
export function syllabify(word: string): string[] {
  ...
  while (i < w.length) {
    ...
    // Nucleus (1 vowel, optionally doubled)
    let nucleus = "";
    if (isVowel(w[i])) {
      nucleus = w[i]; i++;
    ...
    // No vowel found → attach the lone consonant to previous syllable
    if (!nucleus) {
...
/** Needleman–Wunsch over syllables. Gap penalty 1, substitution cost = 1-similarity. */
...
const RULES: Array<{
  match: (exp: string, heard: string) => boolean;
  tip: string;
}> = [
  { // j → z / dz
  ...
  { // ns / nz / nk / nd → simplifié sans nasale
    match: (e, h) => {
      const m = e.match(/^(n[zskdgt])/);
      return !!m && !h.startsWith(m[1]);
    },
  ...
  { // voyelle longue manquée (aa / ee / ii / oo / uu)
...
function deriveTip(expected: string, heard: string): string {
  for (const r of RULES) {
    if (r.match(expected.toLowerCase(), heard.toLowerCase())) return r.tip;
  }
  // Generic fallback
...
export function buildIssues(cells: SyllableCell[]): CoachIssue[] {
  ...
  for (const c of cells) {
    ...
    if (c.status === "missing") {
      issues.push({ expected, heard: "(omis)", tip: `Tu as oublié la syllabe « ${expected} ».` });
    } else if (c.status === "extra") {
      issues.push({ expected: "(en trop)", heard, tip: `Syllabe ajoutée : « ${heard} ».` });
    } else {
      issues.push({ expected, heard, tip: deriveTip(expected, heard) });
    }
  }
  // Dedupe by tip text, keep first 3
...
export function diagnose(expected: string, heard: string): Diagnostic {
  // Per-word diagnosis — for multi-word phrases, evaluate the first lexical word
  // (the rest are mostly grammatical particles).
  ...
  return {
    expectedSyllables: expSyls,
    heardSyllables: heardSyls,
    cells,
    issues: buildIssues(cells),
    score,
    verdict,
  };
}
