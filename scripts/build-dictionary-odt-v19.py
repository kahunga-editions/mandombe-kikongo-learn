...
# ------------------------------------------------------------------ nettoyage
report = Counter()
FR2_SRC = os.environ.get("FR2_CACHE", "/tmp/fr-cache2.json")
fr_cache2 = json.load(open(FR2_SRC)) if os.path.exists(FR2_SRC) else {}
swapped = []
for r in clean:
    if looks_english(r["fr"]) and not r["en"]:
        r["en"] = r["fr"]
        r["fr"] = (fr_cache2.get(r["en"]) or "").strip()
        if not r["fr"]:
            swapped.append(r["en"])
json.dump(sorted(set(swapped)), open("/tmp/missing-fr.json", "w"), ensure_ascii=False)
EN2_SRC = os.environ.get("EN2_CACHE", "/tmp/en-cache2.json")
en_cache2 = json.load(open(EN2_SRC)) if os.path.exists(EN2_SRC) else {}
for r in clean:
    clean_record(r, report)
    # la passe de nettoyage peut avoir retire un point : on retablit la regle
    for f in ("fr", "en"):
        t = r[f]
        if not t:
            continue
        if is_sentence(t, r["lari"]):
            t = t[0].upper() + t[1:]
            if t[-1] not in SENTENCE_END:
                t += "."
        r[f] = t
    if r["lari"]:
        r["lari"] = r["lari"][0].upper() + r["lari"][1:]
...
for r in clean:
    if not r["en"] and r["fr"]:
        alt = (en_cache2.get(r["fr"]) or en_cache2.get(r["lari"]) or "").strip()
        if alt:
            r["en"] = alt

missing_en = [r["lari"] for r in clean if not r["en"]]
json.dump([{"lari": r["lari"], "fr": r["fr"]} for r in clean if not r["en"]],
          open("/tmp/missing-en.json", "w"), ensure_ascii=False)
write_report(os.environ.get("BOOK_REPORT", "/tmp/book-clean-report.txt"),
             report, len(clean), missing_en)
print("Nettoyage :", dict(report), "| sans anglais :", len(missing_en))
