#!/usr/bin/env python3
"""Annexe Kilolaka du livre, bilingue francais / anglais.
...
Nkoma-nkoma attestes par la grille. Aucun sens n'est invente : tout vient de
la grille Kilolaka du site.
"""
import os
import sys
...
def build_annex(grid):
    """Renvoie (xml, nombre de Mazita, residus latins)."""
    groups = {g[0]: g for g in grid["GROUPS"]}
    vowels_all = grid["VOWELS"]
    xml = []
    residues = []
    ...
    xml.append(plain("HeadS", "Notes \u00b7 Notes"))
    for fr, en in zip(CLOSING_FR, CLOSING_EN):
        xml.append(plain("BodyTxt", fr))
        xml.append(plain("BodySmall", "EN \u2014 " + en))
