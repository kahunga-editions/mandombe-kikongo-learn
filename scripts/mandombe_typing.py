...
def to_mandombe(lari: str) -> str:
    ...
    # Le « ia » final se tape tel quel (nkia, kozia, kimfinia). On ne passe par
    # « iya » QUE pour JIA, WIA, PIA, RIA, HIA : ces glyphes n'existent pas dans
    # la police (ex. tilapia -> tilapiya). Aucune autre suite ne recoit « iya ».
    s = re.sub(r"(?i)([jwprh])ia\b", r"\1iya", s)
